/* This function generates an toast message based on the number of invoices
...
const invoicesLengthMessage = (invoicesArr, filterType, windowWidth) => {
    ...
    if (invoicesLength === 0) {return `No Invoices`;}
    else if (invoicesLength === 1) {
        if (isMobile) { return `1 ${type} invoice`;} 
        else { return `There is 1 ${type} invoice`;}
    } else if (invoicesLength > 1) {if (isMobile) { return `${invoicesLength} ${type} invoices`;} 
    else { return `There are ${invoicesLength} ${type} invoices`;}}
};
